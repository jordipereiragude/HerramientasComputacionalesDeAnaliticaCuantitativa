...
/* end function 'timer()' for measuring processor time */
...
int parse(long *n_ad, long *m_ad, node **nodes_ad, 
    ...
    long **cap_ad,
    int InN,int InM,int countSP,int* sP, int* sPV, int* or, int* des, int* lower, int* upper, int* costOut )
...
for ( in = nodes; in <= nodes + n; in ++ ) in -> excess = 0;
...
m_c      = 0;
total_p = total_n = 0;
for ( ndp = nodes; ndp < nodes + n; ndp ++ ) ndp -> excess = 0;
...
int functMain (int InN,int InM,int countSP,int* sP, int* sPV, int* or, int* des, int* lower, int* upper, int* costOut,int* flowRetorno,int* totFlowRetorno)
{
  ...
  parse( &n, &m, &ndp, &arp, &nmin, &c_max, &cap,
    InN, InM, countSP, sP, sPV, or, des, lower, upper, costOut );
...
int main(void)
{
  ...
  int InM=8;
  int countSP=2;
  int* sP=malloc(2*sizeof(int));
  sP[0]=1;
  sP[1]=6;
  int* sPV=malloc(2*sizeof(int));
  sPV[0]=10;
  sPV[1]= -10;
  int* or=malloc(InM*sizeof(int));
  or[0]=1;
  or[1]=1;
  or[2]=2;
  or[3]=3;
  ...
  des[7]=6;
  //des[8]=6;
  int* lower=malloc(InM*sizeof(int));
  lower[0]=0;
  lower[1]=0;
  lower[2]=0;
  lower[3]=0;
  ...
  lower[7]=0;
  //lower[8]=0;
  int* upper=malloc(InM*sizeof(int));
  upper[0]=4;
  upper[1]=8;
  upper[2]=5;
  upper[3]=10;
  ...
  upper[7]=8;
  //upper[8]=100;
  int* costOut=malloc(InM*sizeof(int));
  costOut[0]=1;
  costOut[1]=5;
  costOut[2]=0;
  costOut[3]=1;
  ...
  costOut[7]=1;
  //costOut[8]=0;
  int* flowRetorno=malloc(InM*sizeof(int));
  functMain(InN,InM,countSP,sP,sPV,or,des,lower,upper,costOut,flowRetorno,&totFlowRetorno);
  printf("valor: %d\n",totFlowRetorno);
  for (int h=0;h<InM;h++) printf("(%d %d) --> %d\n",or[h],des[h],flowRetorno[h]);
  return(0);
}
*/
